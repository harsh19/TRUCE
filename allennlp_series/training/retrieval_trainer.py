...
@TrainerBase.register("retrieval")
class Retrieval(TrainerBase):
    def __init__(
        self,
        model: Model,
    ...
    def batch_loss(
        self, batch_group: List[TensorDict], for_training: bool
    ) -> torch.Tensor:
        """
    ...
    def _train_epoch(self, epoch: int) -> Tuple[List, List, List]:  # Dict[str, float]:
        """
        ...
        for batch_group in train_generator_tqdm:
            ...
            batch = batch_group[0]
            batch_features, batch_labels = batch["series"], batch["tokens"]
            # print("batch_features: ", batch_features.size())
            # self.batch_loss(batch_group, for_training=True)
            all_features.extend(batch_features.data.cpu().numpy())
            # all_labels.extend(batch_labels['tokens'].data.cpu().numpy())
            # print(batch.keys())
            if "label" in batch:
                all_labels.extend(batch["label"].data.cpu().numpy())
            # print("metadata: ", batch['metadata'] )
            if type(batch["metadata"][0]["raw_text"]) == str:
                batch_raw_text = [m["raw_text"] for m in batch["metadata"]]
            else:
                batch_raw_text = [m["raw_text"][0] for m in batch["metadata"]]
            all_texts.extend(batch_raw_text)
            # raw_text

        return all_features, all_labels, all_texts

    def _validation_loss(
        self, train_feats, train_labels, train_texts
    ):  # -> Tuple[float, int]:
        """
        ...
        batches_this_epoch = 0
        val_loss = 0
        val_feats, val_labels, val_texts = [], [], []
        idxs = []
        transfer_mode = False
        for batch_group in val_generator_tqdm:
            batch = batch_group[0]
            batch_features, batch_labels = batch["series"], batch["tokens"]
            val_feats.extend(batch_features.data.cpu().numpy())
            if "label" in batch:
                val_labels.extend(batch["label"].data.cpu().numpy())
            if batch["metadata"] is not None:
                if batch["metadata"][0] is not None:
                    #'raw_text' in batch['metadata'][0]:
                    if type(batch["metadata"][0]["raw_text"]) == str:
                        batch_raw_text = [m["raw_text"] for m in batch["metadata"]]
                    else:
                        batch_raw_text = [m["raw_text"][0] for m in batch["metadata"]]
                    val_texts.extend(batch_raw_text)
                    idxs.extend([m["idx"] for m in batch["metadata"]])
        if len(idxs) == 0:
            transfer_mode = True
        #
        #
        ...
        distances, indices = self.model.forward(train_feats, val_feats)

        if len(train_labels) > 0:
            val_predictions_labels = [train_labels[idx[0]] for idx in indices]
        else:
        ...
        if not transfer_mode:
            if self.model.model_name is not None:
                fw = open("tmp/" + self.model.model_name + "/predictions.tsv", "w")
                # vals = ['idx', 'series', 'GT-label', 'one-of-the-references', 'generated text']
                vals = ["idx", "one-of-the-references", "generated text"]
                fw.write(",".join(vals))
                fw.write("\n")
            val_predictions_text = [train_texts[idx[0]] for idx in indices]
            # print(" ---> val_predictions_text = ", val_predictions_text)
            # 0/0
            for val_gt_txti, val_pred_txti, idx in zip(
                val_texts, val_predictions_text, idxs
            ):
                print("val_pred_txti = ", val_pred_txti)
                print("val_gt_txti = ", val_gt_txti)
                print("idx = ", idx)
                self._ngram_overlap_eval(val_pred_txti, val_gt_txti, idx)
                self._diversity_eval(val_pred_txti, typ="generated")
                self._diversity_eval(val_gt_txti, typ="gt")
                if self.model.model_name is not None:
                    vals = [str(idx), val_gt_txti, val_pred_txti]
                    fw.write(",".join(vals))
                    fw.write("\n")
            if self.model.model_name is not None:
                fw.close()

        return val_predictions_labels, val_labels, batches_this_epoch
    ...
    def train(self) -> Dict[str, Any]:
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            train_feats, train_labels, train_texts = self._train_epoch(epoch)
            ...
            if self._validation_data is not None:
                with torch.no_grad():
                    ...
                    val_predictions, val_gt, num_batches = self._validation_loss(
                        train_feats, train_labels, train_texts=train_texts
                    )
                    correct_list = [
                        pred == gt for pred, gt in zip(val_predictions, val_gt)
                    ]
                    correct = sum(correct_list)
                    print("correct: ", correct, " || total: ", len(correct_list))

                    if len(correct_list) > 0:
                        val_metrics = {"accuracy": 100.0 * correct / len(correct_list)}
                    else:
                        val_metrics = {}
                    val_metrics.update(self._ngram_overlap_eval.get_metric(reset=True))
                    val_metrics.update(self._diversity_eval.get_metric(reset=True))
                    # training_util.get_metrics(self.model, val_loss, num_batches, reset=True)

                    # Check validation metric for early stopping
                    if self._validation_metric not in val_metrics:
                        val_metrics[self._validation_metric] = 0.0
